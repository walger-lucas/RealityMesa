...
class UndoVocalCommand(VocalCommands):
    ...
    def activate(sent: Span, info: dict, tt_queue: CommandQueue[Tabletop], ctx_queue: CommandQueue[ContextTask]) -> bool:
        if info["acao"] is not None and info["acao"].lemma_.lower() in UndoVocalCommand.__ACTION_VERB_LEMMA:
            return True
        return False
    @staticmethod
...
vocal_command(UndoVocalCommand,0)