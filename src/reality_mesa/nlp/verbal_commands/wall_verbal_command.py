...
class WallVerbalCommand(VerbalCommands):
    ...
    def activate(sent: Span, info: dict, tt_queue: CommandQueue[Tabletop], ctx_queue: CommandQueue[ContextTask]) -> bool:
        if info["acao"] is not None and info["acao"].lemma_.lower() in WallVerbalCommand.__ACTION_VERB_LEMMA:
...
verbal_command(WallVerbalCommand,0)