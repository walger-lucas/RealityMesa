...
class Tabletop:
    def __init__(self,image:pygame.Surface,tabletop_size:tuple[int,int], unit:str = "m",unit_size:float=1.5):
        self.__id_count = 0
        ...
        self.__tabletop_size = tabletop_size
        self.unit = unit
        ...
        self.remove_list = []
        if (send_future_command(self.vision_queue,StartCamera()).result()):
            self.Calibrate()
    ...
    def Draw(self,surface:pygame.Surface, camera:Camera):
        ...
        camera.DrawWorldSprite(surface,self.__tabletop_image,pygame.Vector2(-0.5,-0.5),pygame.Vector2(self.__tabletop_size))
        offset = pygame.Vector2(-0.5,-0.5)*camera.zoom*camera.pixels_per_unit

        for i in range(0,self.__tabletop_size[0]+1,1):
            posi = camera.World2Screen(pygame.Vector2(i,0))
            posf = camera.World2Screen(pygame.Vector2(i,self.__tabletop_size[1]))
            pygame.draw.line(surface,(255,255,255,150),posi+offset,posf+offset)
        for i in range(0,self.__tabletop_size[1]+1,1):
            posi = camera.World2Screen(pygame.Vector2(0,i))
            posf = camera.World2Screen(pygame.Vector2(self.__tabletop_size[0],i))
            pygame.draw.line(surface,(255,255,255,150),posi+offset,posf+offset)
    ...
    def FixToGrid(self, size:pygame.Vector2,pos: pygame.Vector2) -> pygame.Vector2:
        ...
        pos_grided[0] = max(0,min(self.__tabletop_size[0]-1,pos_grided[0]))
        pos_grided[1] = max(0,min(self.__tabletop_size[1]-1,pos_grided[1]))
    ...
    def DoCalibration(self,surface:pygame.Surface):
        size_sqr = (10,6)
        max_w = (surface.width*0.8)//size_sqr[0]
        max_h = (surface.height*0.8)//size_sqr[1]
